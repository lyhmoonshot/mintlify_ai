---
title: "使用 Kimi API 完成工具调用（tool_calls）"
sidebarTitle: "Tool Calls 说明"
---
...
3. 我们可能会打开一个或多个搜索结果的网页，浏览网页并获取我们需要的知识；
...
_注意：添加到 messages 列表中位于 `role=tool` 的 message 之前的 assistant messages，必须完整包含 Kimi API 返回的 `tool_calls` 字段及字段值。我们推荐直接将 Kimi API 返回的 `choice.message` “原封不动”地添加到 messages 列表中，以避免可能产生的错误。_