...
包含迄今为止对话的消息列表。
这是一个结构体的列表，每个元素类似如下：`json{"role": "user", "content": "你好"}` role 只支持 `system`,`user`,`assistant` 其一，content 不得为空
</ParamField>

<ParamField path="model" required type="string">
Model ID，可以通过 List Models 获取。

目前是 `kimi-k2.5`、`kimi-k2-0905-preview`、`kimi-k2-0711-preview`、`kimi-k2-turbo-preview`、`moonshot-v1-8k`、`moonshot-v1-32k`、`moonshot-v1-128k`、`moonshot-v1-auto`、`moonshot-v1-8k-vision-preview`、`moonshot-v1-32k-vision-preview`、`moonshot-v1-128k-vision-preview` 其一
</ParamField>
