...
[使用 Kimi API 完成工具调用（tool_calls）](/docs/guide/use-kimi-api-to-complete-tool-calls)
...
如果你想避免出现 `finish_reason=length`，我们建议你放大 `max_tokens` 的值，我们推荐的最佳实践是：通过 [estimate-token-count](/docs/api/misc#%E8%AE%A1%E7%AE%97-token) 接口计算输入内容的 Tokens 数量，随后使用 Kimi 大模型所支持的最大 Tokens 数量（例如，对于 `moonshot-v1-32k` 模型，它最大支持 32k Tokens），则可以设置的最大请求的 `max_tokens` 值是 32k。
...
[文件接口](/docs/api/files#%E4%B8%8A%E4%BC%A0%E6%96%87%E4%BB%B6)
