...
上一节的例子中，我们的 `image_url` 为base64编码后的图片。由于视频文件往往更大，所以我们还提供额外方法，您可以先上传图片或视频到 Moonshot，然后通过文件 ID 引用。关于图片或视频的上传，请参阅 [图片理解上传](/api/files)
...
关于上传文件的限制，请参阅 [文件上传](/api/files) 文档。
