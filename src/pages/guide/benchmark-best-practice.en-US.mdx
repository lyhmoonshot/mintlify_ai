...
|  |
|  |
| thinking={`{"type": "enabled"}`} <br /> Recommended max steps = 120 |
|  |
| thinking={`{"type": "enabled"}`} <br /> Recommended max steps = 120 |
| thinking={`{"type": "enabled"}`} <br /> Recommended max steps = 200 <br /> parallel n=8 |
|  |
| thinking={`{"type": "enabled"}`} <br /> Recommended max steps = 120 |
|  |
|  |
| thinking={`{"type": "enabled"}`} <br /> Recommended max steps = 250 <br /> Recommend using a context management mechanism to prevent overly long context and ensure enough tool calls <br /> Include today's date in the system prompt and let the model search when it is uncertain |
| thinking={`{"type": "enabled"}`} <br /> Recommended max steps = 100 |
...
**A.** Your API account has specific rate limits (see [Recharge and Rate Limits](https://platform.moonshot.ai/docs/pricing/limits)). Start low. If you hit **HTTP 429** (rate limit), your concurrency is too high. **Accuracy \> speed,** so tune concurrency to stay within limits.
