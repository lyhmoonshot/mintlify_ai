...
In the previous example, our `image_url` was a base64-encoded image. Since video files are often larger, we provide an additional method where you can first upload images or videos to Moonshot, then reference them via file ID. For information on uploading images or videos, please refer to [Image Understanding Upload](../api/files.en-US.mdx)
...
Images and videos use dynamic token calculation. You can obtain the token consumption of a request containing images or videos through the [estimate tokens API](../api/estimate.en-US.mdx) before starting the understanding process.
...
[Model Inference Pricing](../pricing/chat.en-US.mdx)
...
Regarding file upload limitations, please refer to the [File Upload](../api/files.en-US.mdx) documentation.
...
[Kimi Cli](https://github.com/MoonshotAI/kimi-cli/) is an open source AI Agent by Moonshot.
Kimi Cli has become more powerful with the release of K2.5 model.
[Kimi Agent SDK](https://github.com/MoonshotAI/kimi-agent-sdk/) can be used in your own code, using Kimi Cli more conveniently.

A tool, that can find the source of anime from a screenshot using Kimi Agent SDK is shown as below.
[anime-recognizer](https://github.com/MoonshotAI/kimi-agent-sdk/tree/main/examples/go/anime-recognizer)
